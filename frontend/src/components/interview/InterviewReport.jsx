...
const ReviewCard = ({ entry, index }) => (
  <div className="rounded-[28px] border border-cyan-500/10 bg-slate-950/40 p-6 shadow-[0_18px_40px_rgba(0,0,0,0.3)] backdrop-blur-2xl">
    ...
    <div className="space-y-4">
      ...
      <div>
        ...
        <div className="mt-2 rounded-2xl border border-white/5 bg-slate-950/20 px-4 py-4">
            ...
            {entry.answerReview?.missingConcepts?.length ? (
              <ul className="mt-2 space-y-2 text-sm text-slate-300">
                {entry.answerReview.missingConcepts.map((item) => (
                  <li key={`${entry.question}-${item}`} className="rounded-xl border border-amber-500/20 bg-amber-500/5 px-3 py-2 text-amber-200">
                    {item}
                  </li>
                ))}
              </ul>
      ...
      <div>
        ...
        <div className="mt-2 rounded-2xl border border-cyan-400/20 bg-cyan-500/10 px-5 py-4 text-sm leading-7 text-cyan-100 shadow-[0_8px_30px_rgba(6,182,212,0.15)]">
...
const InterviewReport = ({ session, onReset, actionLabel = "Start another interview" }) => {
  ...
  return (
    <motion.section
      ...
      className="mx-auto w-full max-w-6xl space-y-6 rounded-[32px] border border-cyan-500/10 bg-slate-950/40 p-6 shadow-glow backdrop-blur-2xl sm:p-7 lg:p-8"
    >
      {onReset ? (
        <div className="flex justify-start">
          <motion.button
            ...
            className="rounded-2xl bg-gradient-to-r from-cyan-400 via-sky-400 to-indigo-500 px-6 py-3.5 text-sm font-semibold text-slate-950 shadow-[0_16px_36px_rgba(34,211,238,0.25)] hover:shadow-glow-cyan transition duration-200 hover:scale-[1.01]"
          >
            {actionLabel}
          </motion.button>
        </div>
      ...
      <div className="grid gap-4 lg:grid-cols-[1.3fr_0.7fr]">
        <div className="rounded-[28px] border border-cyan-500/10 bg-slate-950/40 p-6 shadow-[0_22px_60px_rgba(0,0,0,0.3)] backdrop-blur-2xl">
        ...
        <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-1">
          <div className="rounded-[28px] border border-cyan-400/20 bg-cyan-500/10 px-6 py-5 text-center shadow-[0_18px_45px_rgba(34,211,238,0.15)] backdrop-blur-xl">
          ...
          <div className="rounded-[28px] border border-cyan-500/10 bg-slate-950/40 px-5 py-4 shadow-[0_8px_32px_rgba(0,0,0,0.15)] backdrop-blur-xl">
