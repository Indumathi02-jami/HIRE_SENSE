...
const SelectField = ({ label, value, options, onChange }) => (
  <label className="block">
    ...
    <select
      ...
      className="h-14 w-full rounded-2xl clay-input px-4 text-sm text-white outline-none transition-all duration-300"
    >
      {options.map((option) => (
...
const InterviewConfigurator = ({
  ...
  return (
    <motion.section
      ...
      className="clay-card rounded-[32px] p-6 shadow-[0_25px_60px_rgba(0,0,0,0.4)] sm:p-8"
    >
      ...
      <div className="mt-8 space-y-5">
        <label className="block">
          ...
          <div className="rounded-2xl border border-dashed border-cyan-500/30 clay-input p-6 text-center transition-all duration-300 hover:border-cyan-400/40 hover:shadow-glow-cyan/5">
            <input
              type="file"
            ...
            <label htmlFor="resume-upload" className="cursor-pointer">
              <div className="mx-auto mb-3 inline-block rounded-2xl clay-btn-secondary px-5 py-3 text-sm font-semibold text-cyan-200 transition-all duration-200 hover:scale-[1.01]">
        ...
        {isAnalyzing ? (
          <div className="rounded-2xl clay-card p-5 text-sm text-cyan-50 shadow-[0_10px_35px_rgba(6,182,212,0.15)] backdrop-blur-xl">
        ...
        {resumeAnalysis?.resumeProfile ? (
          <div className="rounded-2xl clay-card p-5 shadow-[0_8px_32px_rgba(0,0,0,0.18)] backdrop-blur-xl">
            <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
              ...
              <div className="rounded-full px-4 py-2 text-sm font-semibold text-cyan-200 clay-badge">
            ...
            <div className="mt-4 flex flex-wrap gap-2">
              {[...resumeAnalysis.resumeProfile.technologies, ...resumeAnalysis.resumeProfile.frameworks]
                ...
                .map((item) => (
                  <span
                    key={item}
                    className="rounded-full px-3 py-1.5 text-xs font-semibold text-cyan-200/90 clay-badge"
                  >
                    {item}
                  </span>
                ))}
      ...
      <div className="mt-8 flex flex-wrap items-center gap-3">
        <motion.button
          ...
          className="flex h-14 items-center justify-center gap-3 rounded-2xl clay-btn px-6 text-sm font-semibold text-slate-950 transition disabled:cursor-not-allowed disabled:opacity-70"
        >
          {isStarting ? (
            <>
        ...
        <span className="rounded-full px-4 py-2 text-sm text-cyan-200 clay-badge">
